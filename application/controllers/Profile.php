  ...
  class Profile extends CI_Controller
  {
    ...
    public function change_pass()
      ...
      $user_id=$this->ss->userdata('user_id');
      $d['user']=$this->pf->get_Records($user_id);
      $this->fv->set_rules('new_pass', 'Password', 'trim|required|min_length[8]|max_length[32]');
      $this->fv->set_rules('confirm_pass', 'Password', 'trim|required|min_length[8]|max_length[32]');
      if($this->fv->run()==FALSE)
      {
        $d['x']=1;
        $this->parser->parse('profile',$d);
